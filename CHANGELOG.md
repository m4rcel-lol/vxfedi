...
### Added
- Engagement indicators in post embeds: comment (💬), boost (🔁), and like (❤️)
  counts are prefixed to the embed description and shown on the landing card
- Compact count formatting (`1.5K`, `3.4M`) for large engagement numbers

### Changed
- Boost/repost indicator is now omitted for platforms without a boost mechanism
  (Lemmy/PieFed, Mbin/Kbin, PeerTube) instead of showing a misleading `🔁 0`
